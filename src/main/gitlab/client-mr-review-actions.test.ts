...
describe('gitlab client — MR operations', () => {
  ...
  describe('addMRInlineComment', () => {
    ...
    it('posts an inline discussion with a nested JSON position body', async () => {
      ...
      const [args, options] = glabExecFileAsyncMock.mock.calls[0] as [string[], { stdin?: string }]
      expect(args).toEqual([
        'api',
        '--hostname',
        'git.internal',
  ...
  describe('MR discussion mutations', () => {
    beforeEach(() => {
    ...
    it('replies to a discussion thread', async () => {
      glabExecFileAsyncMock.mockResolvedValueOnce({
        stdout: JSON.stringify({
          id: 501,
      ...
      await expect(
        replyMRDiscussion('/repo', 12, 'discussion-1', 'done', 'upstream', 'conn-1')
      ).resolves.toMatchObject({
        ok: true,
        comment: { id: 501, threadId: 'discussion-1', body: 'done' }
      })
      expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
        [
          'api',
          ...
          '-X',
          'POST',
          'projects/g%2Fp/merge_requests/12/discussions/discussion-1/replies',
          '-f',
          'body=done'
        ],
        {}
    ...
    it('deletes an MR note', async () => {
      glabExecFileAsyncMock.mockResolvedValueOnce({ stdout: '' })

      await expect(deleteMRComment('/repo', 12, 501, 'upstream', 'conn-1')).resolves.toEqual({
        ok: true
      })
      expect(glabExecFileAsyncMock).toHaveBeenCalledWith(
        [
          ...
          '-X',
          'DELETE',
          'projects/g%2Fp/merge_requests/12/notes/501'
        ],
        {}
      )
    })
