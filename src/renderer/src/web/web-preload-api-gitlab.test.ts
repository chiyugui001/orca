...
describe('web GitLab preload API', () => {
  ...
  it('routes every runtime-backed GitLab method through the expected RPC method', async () => {
        ...
        call(method: string, params?: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          if (method === 'status.get') {
            return Promise.resolve({
              ...
              ok: true,
              result: { capabilities: ['gitlab.mrDiscussionMutations.v1'] },
    ...
    const routeCases: {
      ...
      {
        key: 'replyMRDiscussion',
        invoke: (gl) =>
          gl.replyMRDiscussion({
            repoPath,
            iid: 8,
            discussionId: 'discussion-1',
            body: 'Done'
          }),
        expectedMethod: 'gitlab.replyMRDiscussion',
        expectedParams: {
          repoPath,
          repo: repoPath,
          iid: 8,
      ...
      {
        key: 'deleteMRComment',
        invoke: (gl) => gl.deleteMRComment({ repoPath, iid: 8, noteId: 501 }),
        expectedMethod: 'gitlab.deleteMRComment',
        expectedParams: { repoPath, repo: repoPath, iid: 8, noteId: 501 }
      },
      {
        key: 'jobTrace',
    ...
    expect(runtimeCalls.filter((call) => call.method !== 'status.get')).toEqual(
