...
export function createGitLabApi(): WebGitLabApi {
  ...
  const gitLabApi = {
    ...
    deleteMRComment: async (args) => {
      const status = await getRemoteRuntimeStatus().catch(() => null)
      if (!status?.capabilities?.includes(GITLAB_MR_DISCUSSION_MUTATIONS_RUNTIME_CAPABILITY)) {
        return { ok: false, error: GITLAB_MR_DISCUSSION_MUTATIONS_UPDATE_REQUIRED_MESSAGE }
      }
      return route<WebGitLabResult<'deleteMRComment'>>(GITLAB_WEB_RPC_METHODS.deleteMRComment, args)
    },
    ...
    replyMRDiscussion: async (args) => {
      const status = await getRemoteRuntimeStatus().catch(() => null)
      if (!status?.capabilities?.includes(GITLAB_MR_DISCUSSION_MUTATIONS_RUNTIME_CAPABILITY)) {
        return { ok: false, error: GITLAB_MR_DISCUSSION_MUTATIONS_UPDATE_REQUIRED_MESSAGE }
      }
      return route<WebGitLabResult<'replyMRDiscussion'>>(
        GITLAB_WEB_RPC_METHODS.replyMRDiscussion,
        args
      )
    },
